...
Vanilla still owns `m_rangeBuild` + extensions. Workbenches+ **adds** a per-station extra (0–200 m in 50 m steps) in `BuildRangeBonus`, applied in a postfix on `GetStationBuildRange` only. Not a second range formula.

Cycle: look at station, Shift+E (vanilla `AltPlace` + `Use`). ZDO key `WBP_buildExtra`. Stations with `m_rangeBuild <= 0` are skipped (no hammer radius).
